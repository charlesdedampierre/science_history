...
if __name__ == "__main__":
    df = pd.read_csv("data/global_before_1500.csv")
    ...
    df_edge.to_csv("matrix/global_before_1500.csv")
    ...
    df_partition, g = sygma_graph_leiden(
        ...
        edge_bins=10,
        node_bins=10,
        filepath=GRAPH_RESULTS + "/global_before_1500.html",
    )

    df_partition = df_partition.sort_values("community")
    df_partition.to_sql(
        "partition_global_before_1500", conn, if_exists="replace", index=False
    )
