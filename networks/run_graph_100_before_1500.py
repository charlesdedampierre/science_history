...
if __name__ == "__main__":
    ...
    df_final = pd.merge(df_final, df_temporal, on="wikidata_id")
    df_final = df_final[df_final["region_code"].isin(columns_to_keep)]
    # print(df_final.region_code.value_counts())
    ...
    for occupation in df_occupation_min.meta_occupation.unique():
        ...
        if len(my_list) <= 5:
            ...
            selected_elements = list(
                pd.DataFrame(selected_elements).sample(5, random_state=42)[0].values
            )
    ...
    for region in columns_to_keep:
        ...
        if len(df_sample) > 150:
            res = df_sample.sample(150, random_state=42)
        else:
            res = df_sample.copy()
    ...
    df_partition, g = sygma_graph(
        ...
        filepath=GRAPH_RESULTS + "/before_1500_weighted.html",
    )

    df_partition.to_sql(
        "partition_before_1500_weighted", conn, if_exists="replace", index=False
    )
